...
<p>This is called double check, when condition is check twice for object initialization. Is there any problem  with above code? if all thread come at same time before memory is asigned they will wait at synchronized statement. What can be other problem is? suppose first thread get into second if and then it has to do some sort of processing with Singleton instance. Since object is already got memory other new threads will not come into first if statement and will lead to some read-write problem because first thread has not finished it yet.

So someone has to compromise something.</p>      	
